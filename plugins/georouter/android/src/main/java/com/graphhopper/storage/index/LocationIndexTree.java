...
/**
 * This class implements a Quadtree to get the closest node or edge from GPS
 * coordinates. The following properties are different to an ordinary
 * implementation:
 * <ol>
 * <li>To reduce overall size it can use 16 instead of just 4 cell if
 * required</li>
 * <li>Still all leafs are at the same depth, otherwise it is too complicated to
 * calculate the Bresenham line for different resolutions, especially if a leaf
 * node could be split into a tree-node and resolution changes.</li>
 * <li>To further reduce size this Quadtree avoids storing the bounding box of
 * every cell and calculates this per request instead.</li>
 * <li>To simplify this querying and avoid a slow down for the most frequent
 * queries ala "lat,lon" it encodes the point into a reverse spatial key
 * {@see SpatialKeyAlgo} and can the use the resulting raw bits as cell index to
 * recurse into the subtrees. E.g. if there are 3 layers with 16, 4 and 4 cells
 * each, then the reverse spatial key has three parts: 4 bits for the cellIndex
 * into the 16 cells, 2 bits for the next layer and 2 bits for the last layer.
 * It is the reverse spatial key and not the forward spatial key as we need the
 * start of the index for the current layer at index 0</li>
 * <li>An array structure (DataAccess) is internally used and stores the offset
 * to the next cell. E.g. in case of 4 cells, the offset is 0,1,2 or 3. Except
 * when the leaf-depth is reached, then the value is the number of node IDs
 * stored in the cell or, if negative, just a single node ID.</li>
 * </ol>
 *
 * @author Peter Karich
 */
public class LocationIndexTree implements LocationIndex {
    // do not start with 0 as a positive value means leaf and a negative means
    ...
    /**
     * If normed distance is smaller than this value the node or edge is 'identical'
     * and the algorithm can stop search.
     */
    private double equalNormedDelta;

    /**
     * @param g the graph for which this index should do the lookup based on
     *          latitude,longitude.
     */
    ...
    /**
     * Minimum width in meter of one tile. Decrease this if you need faster queries,
     * but keep in mind that then queries with different coordinates are more likely
     * to fail.
     */
    ...
    /**
     * Searches also neighbouring tiles until the maximum distance from the query
     * point is reached (minResolutionInMeter*regionAround). Set to 1 for to force
     * avoiding a fall back, good if you have strict performance and lookup-quality
     * requirements. Default is 4.
     */
    ...
    int calcChecksum() {
        // do not include the edges as we could get problem with CHGraph due to
        // shortcuts
    ...
    /**
     * This method fills the set with stored node IDs from the given spatial key
     * part (a latitude-longitude prefix).
     */
    ...
    // this method returns the spatial key in reverse order for easier
    // right-shifting
    ...
    /**
     * calculate the distance to the nearest tile border for a given lat/lon
     * coordinate in the context of a spatial key tile.
     */
    ...
    /**
     * Calculates the distance to the nearest tile border, where the tile border is
     * the rectangular region with dimension 2*paddingTiles + 1 and where the center
     * tile contains the given lat/lon coordinate
     */
    ...
    public void query(BBox queryShape, final Visitor function) {
        ...
        query(START_POINTER, queryShape, bbox.minLat, bbox.minLon, bbox.maxLat - bbox.minLat, bbox.maxLon - bbox.minLon,
                new Visitor() {
                    @Override
    ...
    final void query(int intPointer, Shape queryBBox, double minLat, double minLon, double deltaLatPerDepth,
            double deltaLonPerDepth, Visitor function, int depth) {
        long pointer = (long) intPointer << 2;
        if (depth == entries.length) {
            int nextIntPointer = dataAccess.getInt(pointer);
            if (nextIntPointer < 0) {
        ...
        for (int cellIndex = 0; cellIndex < max; cellIndex++) {
            ...
            // 2. it picks every second bit (e.g. for just latitudes) and interprets the
            ...
            double tmpMinLon = minLon + deltaLonPerDepth * lonCount, tmpMinLat = minLat + deltaLatPerDepth * latCount;

            BBox bbox = (queryBBox != null || function.isTileInfo())
                    ? new BBox(tmpMinLon, tmpMinLon + deltaLonPerDepth, tmpMinLat, tmpMinLat + deltaLatPerDepth)
                    : null;
            if (function.isTileInfo())
                function.onTile(bbox, depth);
            if (queryBBox == null || queryBBox.contains(bbox)) {
                // fill without a restriction!
                query(nextIntPointer, null, tmpMinLat, tmpMinLon, deltaLatPerDepth, deltaLonPerDepth, function,
                        depth + 1);
            } else if (queryBBox.intersects(bbox)) {
                query(nextIntPointer, queryBBox, tmpMinLat, tmpMinLon, deltaLatPerDepth, deltaLonPerDepth, function,
                        depth + 1);
            }
        }
    }
    ...
    /**
     * This method collects the node indices from the quad tree data structure in a
     * certain order which makes sure not too many nodes are collected as well as no
     * nodes will be missing. See discussion at issue #221.
     * <p>
     *
     * @return true if no further call of this method is required. False otherwise,
     *         ie. a next iteration is necessary and no early finish possible.
     */
    final boolean findNetworkEntries(double queryLat, double queryLon, GHIntHashSet foundEntries, int iteration) {
        ...
        if (iteration % 2 != 0) {
            ...
            if (!foundEntries.isEmpty()) {
                ...
                // Now calculate how far we have to look outside to find any hidden nearest
                // nodes
                // and repeat whole process with wider search area until this distance is
                // covered.
            }
        }
    ...
    public QueryResult findClosest(final double queryLat, final double queryLon, final EdgeFilter edgeFilter) {
        ...
        // denormalize distance and calculate snapping point only if closed match was
        // found
    ...
    /**
     * Returns all edges that are within the specified radius around the queried
     * position. Searches at most 9 cells to avoid performance problems. Hence, if
     * the radius is larger than the cell width then not all edges might be
     * returned.
     * <p>
     * TODO: either clarify the method name and description (to only search e.g. 9
     * tiles) or refactor so it can handle a radius larger than 9 tiles. Also remove
     * reference to 'NClosest', which is misleading, and don't always return at
     * least one value. See map-matching #65. TODO: tidy up logic - see comments in
     * graphhopper #994.
     *
    ...
    public List<QueryResult> findNClosest(final double queryLat, final double queryLon, final EdgeFilter edgeFilter,
            double radius) {
        // Return ALL results which are very close and e.g. within the GPS signal
        // accuracy.
                        ...
                        protected boolean check(int node, double normedDist, int wayIndex, EdgeIteratorState edge,
                            ...
                            if (normedDist < returnAllResultsWithin || queryResults.isEmpty()
                                ...
                                for (int qrIndex = 0; qrIndex < queryResults.size(); qrIndex++) {
                                    ...
                                    // overwrite older queryResults which are potentially more far away than
        ...
        // TODO: pass boolean argument for whether or not to sort? Can be expensive if
        // not required.
    ...
    // make entries static as otherwise we get an additional reference to this class
    // (memory waste)
    ...
    protected abstract class XFirstSearchCheck extends BreadthFirstSearch {
        ...
        protected boolean checkAdjacent(EdgeIteratorState currEdge) {
            ...
            if (!edgeFilter.accept(currEdge)) {
                // only limit the adjNode to a certain radius as currNode could be the wrong
                // side of a valid edge

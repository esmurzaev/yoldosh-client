...
/**
 * This class implements the alternative paths search using the "plateau" and
 * partially the "penalty" method described in the following papers.
 * <p>
 * <ul>
 ...
 * <li>via point 'storage' idea 'Candidate Sets for Alternative Routes in Road
 * Networks', 2013: https://algo2.iti.kit.edu/download/s-csarrn-12.pdf</li>
 * <li>Alternative route graph construction 2011:
 * http://algo2.iti.kit.edu/download/altgraph_tapas_extended.pdf</li>
 * </ul>
 *
 * @author Peter Karich
 */
public class AlternativeRoute implements RoutingAlgorithm {
    ...
    // 1 is default for bidir Dijkstra, 0.8 seems to be a very similar value for
    // bidir A* but roughly 1/2 of the nodes explored
    ...
    static double calcSortBy(double weightInfluence, double weight, double shareInfluence, double shareWeight,
    ...
    /**
     * Increasing this factor results in returning more alternatives. E.g. if the
     * factor is 2 than all alternatives with a weight 2 times longer than the
     * optimal weight are return. (default is 1)
     */
     ...
     * Specify 0.5 to force a same paths of maximum 50%. The unit is the 'weight'
     * returned in the Weighting.
     */
    ...
    /**
     * This method sets the minimum plateau portion of every alternative path that
     * is required.
     */
     ...
     * Default for bidirectional A* is 0.8 (80%). Specify a higher value to get more
     * alternatives (especially if maxWeightFactor is higher than 1.5) and a lower
     * value to improve query time but reduces the possibility to find alternatives.
     */
    ...
    public void setMaxPaths(int maxPaths) {
        ...
        if (this.maxPaths < 2)
            throw new IllegalArgumentException(
                    "Use normal algorithm with less overhead instead if no alternatives are required");
    }

    /**
     * This method calculates best paths (alternatives) between 'from' and 'to',
     * where maxPaths-1 alternatives are searched and they are only accepted if they
     * are not too similar but close to the best path.
     */
    public List<AlternativeInfo> calcAlternatives(int from, int to) {
        AlternativeBidirSearch altBidirDijktra = new AlternativeBidirSearch(graph, weighting, traversalMode,
                maxExplorationFactor * 2);
        ...
        List<AlternativeInfo> alternatives = altBidirDijktra.calcAlternatives(maxPaths, maxWeightFactor, 7,
                maxShareFactor, 0.8, minPlateauFactor, -0.2);
        return alternatives;
    }
    ...
    public static class AlternativeInfo {
        ...
        public AlternativeInfo(double sortBy, Path path, SPTEntry shareStart, SPTEntry shareEnd, double shareWeight) {
            this.sortBy = sortBy;
            this.path = path;
    ...
    public static class AlternativeBidirSearch extends AStarBidirection {
        ...
        public boolean finished() {
            ...
            // The following condition is necessary to avoid traversing the full graph if
            // areas are disconnected
            // but it is only valid for none-CH e.g. for CH it can happen that finishedTo is
            // true but the from-SPT could still reach 'to'
            ...
            // This is more precise but takes roughly 20% longer: return currFrom.weight >
            // bestPath.getWeight() && currTo.weight > bestPath.getWeight();
            // For bidir A* and AStarEdge.getWeightOfVisitedPath see comment in
            // AStarBidirection.finished
        }
        ...
        /**
         * @return the information necessary to handle alternative paths. Note that the
         *         paths are not yet extracted.
         */
        public List<AlternativeInfo> calcAlternatives(final int maxPaths, double maxWeightFactor,
            ...
            // find all 'good' alternatives from forward-SPT matching the backward-SPT and
            // optimize by
            // small total weight (1), small share and big plateau (3a+b) and do these
            ...
            double bestShare = 0;
            double sortBy = calcSortBy(weightInfluence, bestPath.getWeight(), shareInfluence, bestShare,
                    plateauInfluence, bestPlateau);

            final AlternativeInfo bestAlt = new AlternativeInfo(sortBy, bestPath, bestPath.sptEntry, bestPath.edgeTo,
                    bestShare);
            ...
            bestWeightMapFrom.forEach(new IntObjectPredicate<SPTEntry>() {
                ...
                public boolean apply(final int traversalId, final SPTEntry fromSPTEntry) {
                    ...
                    if (traversalMode.isEdgeBased()) {
                        if (toSPTEntry.parent != null)
                            ...
                            // 1. make only turn costs missing in 'weight' and not duplicating current
                            // edge.weight
                    ...
                    if (tmpFromEntry == null || tmpFromEntry.parent == null) {
                        // we can be here only if edge based and only if entry is not part of the best
                        // path
                        // e.g. when starting point has two edges and one is part of the best path the
                        // other edge is path of an alternative
                    ...
                    // that the from-SPTEntry is the start of the plateau or there is no plateau at
                    // all
                    //
                    ...
                    while (prevToSPTEntry.parent != null) {
                        int nextFromTraversalId = traversalMode.createTraversalId(prevToSPTEntry.adjNode,
                                prevToSPTEntry.parent.adjNode, prevToSPTEntry.edge, false);
                        ...
                        plateauWeight += (prevToSPTEntry.getWeightOfVisitedPath()
                                - prevToSPTEntry.parent.getWeightOfVisitedPath());
                    ...
                    if (smallShare) {
                        double sortBy = calcSortBy(weightInfluence, weight, shareInfluence, shareWeight,
                                plateauInfluence, plateauWeight);
                        ...
                        if (sortBy < worstSortBy || alternatives.size() < maxPaths) {
                            Path path = new PathBidirRef(graph, weighting).setSPTEntryTo(toSPTEntry)
                                    .setSPTEntry(fromSPTEntry).setWeight(weight);
                            path.extract();

                            // for now do not add alternatives to set, if we do we need to remove then on
                            // alternatives.clear too (see below)
                            // AtomicInteger tid = addToMap(traversalIDMap, path);
                            // int tid = traversalMode.createTraversalId(path.calcEdges().get(0), false);
                            alternatives.add(new AlternativeInfo(sortBy, path, fromEE, toEE, shareWeight));
                ...
                SPTEntry getFirstShareEE(SPTEntry startEE, boolean reverse) {
                    while (startEE.parent != null) {
                        ...
                        int tid = traversalMode.createTraversalId(startEE.adjNode, startEE.parent.adjNode, startEE.edge,
                                reverse);
                ...
                boolean isBestPath(SPTEntry fromSPTEntry, Path bestPath) {
                    if (traversalMode.isEdgeBased()) {
                        if (GHUtility.getEdgeFromEdgeKey(startTID.get()) == fromSPTEntry.edge) {
                            if (fromSPTEntry.parent == null)
                                throw new IllegalStateException(
                                        "best path must have no parent but was non-null: " + fromSPTEntry);
                        ...
                        if (startTID.get() != fromSPTEntry.adjNode)
                            throw new IllegalStateException("Start traversal ID has to be identical to root edge entry "
                                    + "which is the plateau start of the best path but was: " + startTID
                                    + " vs. adjNode: " + fromSPTEntry.adjNode);
        ...
        /**
         * This method adds the traversal IDs of the specified path as set to the
         * specified map.
         */
        AtomicInteger addToMap(GHIntObjectHashMap<IntSet> map, Path path) {
            ...
            for (EdgeIteratorState iterState : path.calcEdges()) {
                ...
                set.add(tid);
                if (startTID.get() < 0) {
                    // for node based traversal we need to explicitely add base node as starting
                    // node and to list
                    if (!traversalMode.isEdgeBased()) {
